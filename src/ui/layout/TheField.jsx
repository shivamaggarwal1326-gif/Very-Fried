...
export default function TheField({ userXP, isAnonymous, onRequireAuth, onFieldCheckout }) {
  ...
  // --- THE VIP LOCK ---
  // In Phase 5, this will fetch from the backend to see if user visits >= 6
  ...
  useEffect(() => {
    const fetchElitePartners = async () => {
      setIsLoading(true);
      try {
        const { data, error } = await supabase.from('merchants').select('id, business_name, partnership_tier').eq('partnership_tier', 'ELITE_PARTNER'); 
  ...
  useEffect(() => {
    ...
    const fetchMenu = async () => {
      const { data } = await supabase.from('recipes').select('*').eq('merchant_id', selectedRest.id);
      if (data) {
        const categories = [...new Set(data.map(r => r.data.category || 'MUNCHIES'))];
        const formattedMenu = categories.map(cat => ({
          title: cat,
          items: data.filter(r => (r.data.category || 'MUNCHIES') === cat).map(r => ({
              id: r.id, name: r.data.title, price: `₹${r.data.price}`, desc: r.data.desc || r.data.description, diet: r.data.diet || 'VEG' 
            }))
        }));
  ...
  const deployCode = () => {
    if (isAnonymous && onRequireAuth) { onRequireAuth(); return; }
    
    // Ghost Table Logic: If they are VIP, bypass the activeTableId requirement for remote takeaway
    if ((activeTableId || isVIP) && onFieldCheckout) {
      const fullLoadout = tacticalLoadout.map(item => {
        let itemCategory = 'MUNCHIES';
        selectedRest.menuCategories.forEach(cat => { if (cat.items.some(i => i.name === item.name)) itemCategory = cat.title; });
        return { ...item, category: itemCategory };
      });
      if (onFieldCheckout(fullLoadout)) setTacticalLoadout([]);
    }
  };
  ...
  return (
    <div className="w-full flex-grow flex flex-col relative z-10 pb-40 md:pb-12 pt-4">
            ...
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} className="bg-[#fcfbf9] border-4 border-black p-6 md:p-8 max-w-md w-full shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] pointer-events-auto">
              ...
              <div className="flex flex-col gap-3">
                {SPICE_GRADES.map((grade) => (
                  <button key={grade.level} onClick={() => confirmSpiceSelection(grade)} className={`w-full p-4 border-2 border-black flex justify-between items-center group transition-transform hover:translate-x-1 hover:-translate-y-1 ${grade.color} ${grade.text}`}>
      ...
      {!selectedRest ? (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-6 w-full max-w-5xl mx-auto px-4 md:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {isLoading ? (
              <div className="col-span-full py-20 text-center font-black uppercase text-gray-400 animate-pulse tracking-widest">Scanning Deployment Zones...</div>
            ) : partneredRestaurants.map((rest) => (
              <motion.div key={rest.id} whileHover={{ x: 4, y: -4, boxShadow: "6px 6px 0px 0px rgba(249,115,22,1)" }} onClick={() => { setSelectedRest({ ...rest, menuCategories: [] }); setTacticalLoadout([]); useStore.setState({ viewingMerchantId: rest.id }); }} className={`border-4 border-black p-6 bg-[#fcfbf9] cursor-pointer group flex flex-col transition-all shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]`}>
                <div className="flex justify-between items-start mb-4"><span className={`bg-orange-500 text-black border-2 border-black font-black text-[10px] md:text-xs px-3 py-1.5 uppercase tracking-widest`}>VERIFIED PARTNER</span></div>
                ...
                <div className="mt-auto pt-4 flex items-center gap-2"><div className="w-2 h-2 bg-orange-500 rounded-full animate-pulse border border-black"></div><span className="font-black text-sm uppercase text-gray-600">Secure Protocol Active</span></div>
              </motion.div>
            ))}
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="flex flex-col w-full max-w-4xl mx-auto px-4 md:px-8 relative pb-20">
          <button onClick={() => { setSelectedRest(null); useStore.setState({ viewingMerchantId: null }); }} className="text-black text-[10px] md:text-xs font-black uppercase tracking-widest hover:text-orange-600 transition-colors self-start mb-6 flex items-center gap-2">
          ...
          <div className="border-4 border-black bg-[#fcfbf9] shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] flex flex-col mb-8 relative overflow-hidden min-h-[500px]">
            <div className="flex flex-col border-b-4 border-black p-6 md:p-10 pb-4 shrink-0 bg-white">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-sm md:text-base font-black text-gray-500 uppercase tracking-widest">VIP PERK: <span className="text-orange-600">Elite Status Verified</span></p>
                </div>
              </div>

              {!isVIP && (
                <div className="bg-yellow-100 border-2 border-yellow-500 p-4 mt-6 flex flex-col justify-center shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] relative overflow-hidden">
                  <div className="absolute -right-4 -bottom-4 opacity-10 text-6xl">🔥</div>
                  <p className="text-sm md:text-base font-black uppercase tracking-widest text-black z-10">
                    Tap the NFC tag & Dine your Mood :)
                  </p>
                  <p className="text-[10px] font-bold text-yellow-800 uppercase mt-1 z-10">Reach Level 6 (VIP) to unlock remote Takeaway.</p>
                </div>
              )}
            </div>

            <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-gray-50">
              {selectedRest.menuCategories && selectedRest.menuCategories.length > 0 ? (
                selectedRest.menuCategories.map((category, catIdx) => (
                  <div key={catIdx} className="mb-8">
                    <h3 className="text-2xl font-black uppercase tracking-tighter mb-4 border-b-4 border-black pb-2">{category.title}</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {category.items.map((item, idx) => (
                        <div key={idx} className="bg-white border-2 border-black p-4 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-col justify-between transition-transform hover:-translate-y-1">
                          <div>
                            <div className="flex justify-between items-start mb-2">
                              <h4 className="font-black uppercase text-lg leading-tight flex items-start"><DietBadge diet={item.diet} />{item.name}</h4>
                              <span className="font-black text-orange-600">{item.price}</span>
                            </div>
                            <p className="text-xs font-bold text-gray-500 uppercase tracking-widest line-clamp-2 mb-4">{item.desc}</p>
                          </div>
                          
                          {/* ONLY VIPs see the Add to Cart button */}
                          {isVIP && (
                            <button onClick={() => setItemPendingSpice({ item })} className="w-full bg-black text-white font-black uppercase text-xs tracking-widest py-2 border-2 border-black shadow-[2px_2px_0px_0px_rgba(249,115,22,1)] hover:shadow-none hover:translate-y-1 hover:bg-orange-500 transition-all mt-auto">
                              + ADD TO LOADOUT
                            </button>
                          )}
                        </div>
                ...
                <div className="text-center py-20 text-gray-400 font-black uppercase tracking-widest flex flex-col items-center">
      ...
      {/* Cart only appears if you have items AND you are a VIP */}
      <AnimatePresence>
        {tacticalLoadout.length > 0 && selectedRest && isVIP && (
          <div className="fixed inset-0 pointer-events-none z-50 flex justify-center">
            <div className="w-full max-w-5xl h-full relative">
              <motion.div initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 100, opacity: 0 }} className="absolute bottom-0 right-0 md:bottom-8 md:right-4 lg:right-0 w-full md:w-[420px] bg-white border-t-4 md:border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex flex-col max-h-[85vh] pointer-events-auto">
                <div className="bg-black text-white px-4 py-3 flex justify-between items-center shrink-0">
                  <h3 className="font-black uppercase tracking-widest text-lg">VIP Takeaway</h3>
                    ...
                    <div key={idx} className="flex justify-between items-start border-2 border-gray-200 bg-white p-3 shadow-sm relative group">
                      <div className="flex flex-col pr-8">
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          <button onClick={() => setItemPendingSpice({ item, editIndex: idx })} className={`text-[9px] font-black uppercase px-2 py-1 ${item.spice.color} ${item.spice.text} border border-black`}>{item.spice.name} ✎</button>
                ...
                <div className="p-4 bg-white border-t-2 border-gray-200">
                   <button onClick={deployCode} className="w-full bg-orange-500 text-black font-black uppercase tracking-widest py-4 text-lg border-4 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-y-1 hover:shadow-none transition-all">TRANSMIT VIP ORDER</button>
                </div>
              </motion.div>
            </div>
          </div>
